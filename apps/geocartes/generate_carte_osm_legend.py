...
LOGO_CACHE_DIR = HERE / "../../ressources/clubs"
...
# ---------------------------------------------------------------------------
def download_logo_bytes(name: str, url: str, timeout: int = 8):
...
def draw_team_legend(fig, found):
    """Ajoute en bas de la figure une légende en tableau pour les clubs.

    Chaque case contient le logo du club lorsqu'il est disponible, puis
    son nom court. La légende est utilisée uniquement en mode --teams.
    """
    if not found:
        return

    # 3 colonnes pour garder une légende compacte, avec adaptation au nombre
    # de clubs.
    columns = 3 if len(found) >= 5 else 2
    rows = math.ceil(len(found) / columns)

    # Hauteur dynamique de la zone de légende.
    legend_height = min(0.30, max(0.105, 0.055 * rows + 0.045))
    bottom = 0.015

    # Réserve la place de la légende sous la carte.
    map_bottom = bottom + legend_height + 0.015
    ax_map = fig.axes[0]
    ax_map.set_position([0.035, map_bottom, 0.93, 0.965 - map_bottom])

    ax_legend = fig.add_axes([0.035, bottom, 0.93, legend_height])
    ax_legend.set_xlim(0, columns)
    ...
    # Les cellules sont placées sous le titre.
    title_offset = 0.30
    available_rows = max(rows, 1)

    for i, (nom, _cx, _cy, pil_image) in enumerate(found):
        col = i % columns
        row = i // columns
        y_top = rows - row - title_offset

        # Cellule blanche avec bordure, style tableau/légende.
        cell = plt.Rectangle(
            (col + 0.01, y_top - 0.86),
            0.98,
            0.78,
        ...
        ax_legend.add_patch(cell)

        # Petit logo à gauche.
        if pil_image is not None:
            oi = OffsetImage(pil_image, zoom=0.11)
            ab = AnnotationBbox(
                oi,
                (col + 0.13, y_top - 0.47),
                xycoords=ax_legend.transData,
            ...
            # Initiales en remplacement du logo si celui-ci n'est pas disponible.
...
def main():
    ...
    parser.add_argument(
        ...
        default="osm",
        help=(
            "Fond de carte : 'osm' pour OpenStreetMap (défaut), "
            "ou 'none' pour le fond uni historique."
        ),
    )
    ...
    if args.map_style == "osm" and not HAS_CONTEXTILY:
        sys.exit(
            "Le fond OpenStreetMap nécessite 'contextily'.\n"
    ...
    if args.teams:
        ...
        for name in items_requested:
            ...
            pil_image = None
            if logo_url:
                raw = download_logo_bytes(club_key, logo_url)
    ...
    for code, slug, dept_geo, communes_geo, index in depts_data:
        for feature in communes_geo["features"]:
            for ring in iter_polygons(feature["geometry"]):
                ...
                ax.fill(
                    ...
                    linewidth=0.25,
                    alpha=0.22 if args.map_style == "osm" else 1.0,
                    zorder=2,
                )
        ...
        for ring in iter_polygons(dept_geo["geometry"]):
            xs = [p[0] for p in ring]
            ys = [p[1] for p in ring]
            ax.plot(xs, ys, color=COLOR_DEPT_BORDER, linewidth=2.2, zorder=4)
    ...
    # ------------------------------------------------------------------
    # Fond cartographique détaillé OpenStreetMap
    #
    # Les GeoJSON du script sont en longitude/latitude (EPSG:4326).
    # contextily récupère les tuiles et les reprojette automatiquement.
    # ------------------------------------------------------------------
    if args.map_style == "osm":
        try:
            ctx.add_basemap(
                ax,
            ...
            sys.exit(
                "Impossible de charger le fond OpenStreetMap.\n"
                f"Détail : {exc}\n"
                "Vérifiez votre connexion Internet ou utilisez --map-style none."
            )
    ...
    for nom, cx, cy, pil_image in found:
        if args.teams:
            ...
            # En mode communes, on conserve les noms sous les épingles.
    ...
    if not args.no_inset:
        draw_inset_locator(
            fig,
            HERE / "departements-france.geojson",
            {c for c, s, *_ in depts_data},
        )

    # # En mode --teams, les noms ne sont plus affichés sous les logos :
    # # ils sont regroupés dans une légende/tableau en bas de la carte.
    # if args.teams:
    #     draw_team_legend(fig, found)
